...
String EspClass::getResetReason(void)
{
	...
	else if (resetInfo.reason == REASON_WDT_RST)
	{ // hardware watch dog reset
		strcpy_P(buff, PSTR("Hardware Watchdog"));
	}
	else if (resetInfo.reason == REASON_EXCEPTION_RST)
	{ // exception reset, GPIO status won’t change
		strcpy_P(buff, PSTR("Exception"));
	}
	else if (resetInfo.reason == REASON_SOFT_WDT_RST)
	{ // software watch dog reset, GPIO status won’t change
		strcpy_P(buff, PSTR("Software Watchdog"));
	}
	else if (resetInfo.reason == REASON_SOFT_RESTART)
...
bool EspClass::flashWrite(uint32_t offset, uint32_t *data, size_t size)
{
	...
	int rc;
	uint32_t *ptr = data;
	if ((flash_chip_id & 0x000000ff) == 0x85)
	{ // 0x146085 PUYA
